...
def main():
    db = SessionLocal()
    try:
        print("Clearing appointments, availability, and doctors...")
        ...
        print("Seeding new doctor records...")
        for doc in doctors:
            db.add(doc)
        db.commit()

        print("Generating 7 days of availability slots...")
        ...
        for i in range(7):
            ...
            for doc in doctors:
                ...
                for idx, (start, end) in enumerate(slots):
                    # Demonstration setup (all available by default for the new list)
                    status = "AVAILABLE"
